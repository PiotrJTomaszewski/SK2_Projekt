...
void server_game_start_game(struct ROOM *room) {
    ...
    if (rand() % 2) {
        ...
        ser_cli_com_send_message(room->player_one, SCMSG_YOU_PLAY_LIGHT, 0, 0);
        ser_cli_com_send_message(room->player_two, SCMSG_YOU_PLAY_DARK, 0, 0);
    } else {
        room->player_one->player_color = COLOR_DARK;
        room->player_two->player_color = COLOR_LIGHT;
        ser_cli_com_send_message(room->player_one, SCMSG_YOU_PLAY_DARK, 0, 0);
        ser_cli_com_send_message(room->player_two, SCMSG_YOU_PLAY_LIGHT, 0, 0);
    }
    ser_cli_com_send_message(room->player_one, SCMSG_LIGHT_TURN, 0, 0);
    ser_cli_com_send_message(room->player_two, SCMSG_LIGHT_TURN, 0, 0);
}
